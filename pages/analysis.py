...
def show():
    ...
    freq_options = {'Beta-Binomial': 0, 'Poisson': 1}

    fitted_freq_model = st.selectbox("Fit data using distribution function:",
                                     options=freq_options,
                                     index=freq_options[st.session_state.freq_model])
    if fitted_freq_model == 'Poisson':
        mle = sum(df[const.HAS_CLAIM]) / n
        st.write("For Poisson distribution, the MLE is  "
                 r"$\small {\displaystyle {\widehat {\lambda }}_{\mathrm {MLE} }={\frac {1}{n}}\sum _{i=1}^{n}x_{i}.}$"
                 " In our dataset:")
        st.write(fr"""
        - n = {df.shape[0]:,.0f}
    ...
    if fitted_freq_model == 'Beta-Binomial':
        mle = sum(df[const.HAS_CLAIM]) / n
    ...
    if st.session_state.freq_model == 'Beta-Binomial':
    ...
    cl = st.number_input('Confidence level: ', 0.5, 0.999, 0.99, format='%.3f')
    st.write("""
